...
export function PrevNextNav({ category, prev, next, lang, t }) {
  if (!prev && !next) return null;

  return (
    <nav className="prev-next-nav">
      {prev ? (
        <Link to={`/algorithms/${category}/${prev.slug}`} className="prev-next-nav__link prev-next-nav__link--prev">
          <span className="prev-next-nav__label">← {t.prev}</span>
          <span className="prev-next-nav__title">{prev.name[lang] ?? prev.name.ru}</span>
        </Link>
      ) : <span />}
      {next ? (
        <Link to={`/algorithms/${category}/${next.slug}`} className="prev-next-nav__link prev-next-nav__link--next">
