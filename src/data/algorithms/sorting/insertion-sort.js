export const insertionSort = {
  ...
  details: {
    deepDive: {
      ru: 'Сортировка вставками работает как естественный процесс построения упорядоченного целого из неупорядоченных частей. Алгоритм мысленно делит массив на отсортированную левую часть (растущую) и неотсортированную правую (сжимающуюся). На каждом шаге берётся первый элемент неотсортированной части - это ключ, который нужно вставить. Затем алгоритм идёт влево по отсортированной части, сдвигая каждый элемент на одну позицию вправо, если он больше текущего ключа. Как только встречается элемент меньший или равный ключу (или достигается начало массива), в освободившееся место встраивается ключ. Отличительная особенность: адаптивность к исходному порядку. Если массив почти отсортирован, каждый ключ окажется недалеко от своей окончательной позиции, поэтому сдвигов будет мало. На обратно отсортированном массиве ключ будет идти влево через весь диапазон, что даёт худший случай O(n²). Число сдвигов равно числу инверсий в массиве.',
      en: 'Insertion sort works like the natural process of building an ordered whole from disordered parts. The algorithm conceptually splits the array into a growing sorted left part and a shrinking unsorted right part. On each step, the first element of the unsorted part is taken - this is the key to be inserted. The algorithm then walks leftward through the sorted part, shifting each element one position right if it is larger than the key. As soon as an element smaller than or equal to the key is found (or the start is reached), the key is placed in the freed spot. The distinguishing feature: adaptivity to input order. If the array is nearly sorted, each key will be near its final position, so few shifts occur. On a reverse-sorted array, the key walks leftward through the entire range, yielding O(n²) worst case. The shift count equals the number of inversions.',
    },
    whenToUse: {
      ru: 'Сортировка вставками ценна в четырёх сценариях. Во-первых, на почти отсортированных данных - если в массиве мало инверсий, алгоритм будет близок к O(n), что быстрее чем Mergesort или Quicksort на таких данных. Во-вторых, как онлайн-алгоритм: если данные поступают потоком, элемент за элементом, вставки по одному - это фактически то, что происходит (в отличие от Selection Sort, который требует предварительного знания всех элементов). В-третьих, на малых подмассивах (< 10-20 элементов) внутри гибридных алгоритмов - это финальный шаг Timsort (Python), Introsort (C++ STL) и других, где вычисленные относительно отсортированные куски потом объединяются. В-четвёртых, в системах, где память критична и нужна O(1) дополнительного места. Сравнение с Selection Sort: последняя всегда делает n-1 перестановку независимо от входа, вставки же адаптируются к числу инверсий. На связных списках вставки особенно эффективны, так как вставка узла - это O(1), хотя поиск позиции остаётся O(n).',
      en: 'Insertion sort has value in four scenarios. First, on nearly-sorted data - if the array has few inversions, the algorithm approaches O(n), faster than Mergesort or Quicksort on such data. Second, as an online algorithm: if data arrives in a stream, one element at a time, inserting one by one is essentially what happens (unlike Selection Sort, which needs all elements known upfront). Third, on small subarrays (< 10-20 elements) inside hybrid algorithms - this is the final step of Timsort (Python), Introsort (C++ STL), and others, where relatively sorted chunks are then merged. Fourth, in memory-critical systems needing O(1) extra space. Comparing to Selection Sort: the latter always does n-1 swaps regardless of input, while insertions adapt to the inversion count. On linked lists insertions are particularly efficient, since inserting a node is O(1), though finding the position remains O(n).',
    },
    realWorld: {
      ru: 'Чистую сортировку вставками встречают в production редко, но её идеи везде. Timsort, стандарт Python (3.x) и Java (для объектов), работает так: находит естественные уже отсортированные куски (runs) в массиве, затем разбивает длинные куски на размер ~64 элемента и применяет Insertion Sort к каждому, затем сливает куски через Merge Sort. Это использует адаптивность: на почти отсортированных данных алгоритм обнаруживает длинные runs и делает несколько merges; на случайных данных делает много маленьких insertion-sort шагов и больше merges. Introspective Sort (Introsort) в C++ STL и Rust начинает с Quicksort, переключается на Heapsort при глубокой рекурсии, потом финиширует Insertion Sort. Также идея появляется в адаптивных алгоритмах поиска: бинарный поиск с Insertion Sort для записей близко к месту вставки. На базах данных: индексы B-tree используют концептуально похожую идею - листья содержат отсортированные записи, вставка поддерживает порядок через перестановки и расщепления узлов. В конкурсах программирования иногда используется для сортировки связных списков, где она быстрее других O(n²) алгоритмов благодаря O(1) вставке узла.',
      en: 'Pure insertion sort appears in production code rarely, but its ideas are everywhere. Timsort, Python\'s standard (3.x) and Java\'s (for objects), works by finding natural already-sorted chunks (runs) in the array, splitting long runs to ~64 elements and applying Insertion Sort to each, then merging chunks via Merge Sort. This leverages adaptivity: on nearly-sorted data the algorithm finds long runs and does few merges; on random data it does many small insertion-sort passes and more merges. Introspective Sort (Introsort) in C++ STL and Rust starts with Quicksort, switches to Heapsort at deep recursion, then finishes with Insertion Sort. The idea also appears in adaptive search algorithms: binary search with Insertion Sort for records close to insertion point. In databases: B-tree indices use a conceptually similar idea - leaves hold sorted records, insertion maintains order through swaps and node splits. In competitive programming, it is sometimes used to sort linked lists, where it outperforms other O(n²) algorithms thanks to O(1) node insertion.',
    },
  },
