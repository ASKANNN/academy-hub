  ...
  details: {
    ...
    whenToUse: [
      ...
      },
      {
        ...
        en: '**Three decision axes** sum up the trade-off against its neighbors (the swap-vs-comparison compromise itself is covered in "How it works"): swaps, comparisons, stability. The practical takeaway - reach for selection sort only when minimizing writes clearly outweighs both speed on nearly-sorted data and preserving the order of equal elements.',
      },
      {
      ...
      {
        ru: 'Нужен алгоритм с той же идеей «извлеки минимум, поставь на место», но без O(n²)? Тогда искать нужно не замену сортировке выбором, а её ускоренную версию - **Heapsort**, разобранную в разделе «Примеры в коде». Сама сортировка выбором в этом случае не применяется - она лишь подсказывает, в какую сторону думать.',
        en: 'Need the same "extract the minimum, place it" idea without O(n²)? Then the answer is not a replacement for selection sort but its sped-up version - **Heapsort**, covered in the "In code" section. Selection sort itself is not used here - it just points the way.',
      },
    ],
    realWorld: [
      {
      ...
      },
      {
        ...
        en: '**Stable selection sort** (shift instead of swap - the mechanics are covered in "How it works") shows up in algorithms courses as the classic "why stability costs writes" exercise, e.g. in CLRS problem sets and on GeeksforGeeks. It never caught on in production: wherever stability is needed, standard libraries (Python, Java, C++) reach for **Timsort**, not stable selection sort.',
      },
      {
