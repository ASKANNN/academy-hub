export const selectionSort = {
  ...
  details: {
    deepDive: {
      ru: 'Сортировка выбором разбивает массив на две концептуальные части: отсортированную (слева) и неотсортированную (справа). На каждом шаге алгоритм сканирует всю неотсортированную часть с целью найти минимальный элемент - не в смысле нескольких сравнений, а именно просмотра всех оставшихся элементов. Как только минимум найден, он обменивается местами с первым элементом неотсортированной части (то есть с границей), и граница сдвигается на один элемент вправо. Это гарантирует ровно одну перестановку на каждом из n-1 шагов, независимо от исходного порядка данных. Ключевое отличие от пузырьковой: сортировка выбором всегда проходит весь оставшийся диапазон, тогда как пузырьковая может "рано выйти", если массив уже отсортирован. Однако сортировка выбором никогда не может воспользоваться этой оптимизацией - число сравнений остаётся n(n-1)/2.',
      en: 'Selection sort conceptually splits the array into a sorted part (left) and an unsorted part (right). On each step, the algorithm scans the entire unsorted part to find the minimum element - not just a few comparisons, but a full pass through all remaining elements. Once found, it swaps with the first element of the unsorted part (the boundary) and advances the boundary one position right. This guarantees exactly one swap per step across n-1 steps, regardless of the input order. Key difference from bubble sort: selection sort always scans the full remaining range, whereas bubble sort can "exit early" if the array is already sorted. However, selection sort can never exploit this optimization - the comparison count stays at n(n-1)/2.',
    },
    whenToUse: {
      ru: 'Сортировку выбором редко используют в production-коде для больших данных, но она ценна в трёх нишах. Во-первых, когда операции записи значительно дороже чтения - например, при записи на flash-память, EEPROM или когда элементы - это тяжёлые объекты (физические записи в БД). В таких сценариях минимизация числа обменов со стоимостью O(n) может перевесить лишние сравнения. Во-вторых, как шаг в гибридных алгоритмах: Heapsort можно рассматривать как ускоренную версию выбора, использующую структуру данных (кучу) для быстрого поиска минимума. В-третьих, для очень малых массивов (< 10 элементов), когда простота кода важнее асимптотики, хотя даже там часто предпочитают Insertion Sort за меньшее число операций. Сравнение с пузырьковой: последняя может быть быстрее на почти отсортированных данных благодаря флагу ранней остановки, а сортировка выбором останется O(n²) в любом случае.',
      en: 'Selection sort is rarely used in production for large data, but it has value in three niches. First, when write operations are significantly more expensive than reads - for instance, writing to flash, EEPROM, or when elements are heavy objects (database record writes). In such scenarios, minimizing swaps at O(n) cost can outweigh extra comparisons. Second, as a step in hybrid algorithms: Heapsort can be seen as an accelerated version of selection using a data structure (a heap) to speed up finding the minimum. Third, for very small arrays (< 10 elements) where code simplicity outweighs asymptotics, though even there Insertion Sort is often preferred for fewer operations. Comparing to bubble sort: the latter can be faster on nearly-sorted data thanks to early exit, while selection sort stays O(n²) no matter what.',
    },
    realWorld: {
      ru: 'Чистую сортировку выбором встречают редко, но её философия живит несколько практических алгоритмов. Heapsort (пирамидальная сортировка) - это фактически сортировка выбором, которая вместо линейного поиска минимума за O(n) использует кучу для O(log n) извлечения. Python 3 и Java используют Timsort, который на малых подмассивах переключается на Insertion Sort (более эффективный вариант, так как работает на месте с меньшим числом перестановок). Embedded системы, работающие с flash-памятью (микроконтроллеры, IoT), могут использовать сортировку выбором или её вариантов при очень маленьких размерах данных, чтобы свести число записей к минимуму. Старые версии V8 (JavaScript движок Chrome) до оптимизаций содержали простую сортировку выбором для массивов < 10 элементов. Также она практикуется в соревнованиях по программированию и олимпиадах как классический пример алгоритма с неудобным свойством: все три случая сложности (лучший, средний, худший) одинаковы.',
      en: 'Pure selection sort appears rarely in practice, but its philosophy underlies several real algorithms. Heapsort is essentially selection sort that uses a heap to extract the minimum in O(log n) instead of O(n) linear search. Python 3 and Java use Timsort, which switches to Insertion Sort for small subarrays (more efficient as it works in-place with fewer swaps). Embedded systems dealing with flash memory (microcontrollers, IoT) might use selection sort or its variants for tiny datasets to minimize writes. Older V8 (Chrome JS engine) before optimizations contained simple selection sort for arrays < 10 elements. It also appears in competitive programming and olympiads as a classic example of an algorithm with an inconvenient property: all three complexity cases (best, average, worst) are identical.',
    },
  },
