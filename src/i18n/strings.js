...
export const STRINGS = {
    ru: {
        ...
        algorithms: {
            ...
            bigOReadoutN: (n) => `При n = ${n}:`,
            bigOReadoutCase: (label, value, opsWord) => `${label}: ≈ ${value} ${opsWord}`,
        ...
        architecturalPatterns: {
            breadcrumbHome: 'Askan Academy',
            breadcrumbArchitecturalPatterns: 'Architectural Patterns Academy',
            catalogEyebrow: 'Software Engineering Platform',
            catalogTitle: 'Architectural Patterns Academy',
            catalogSubtitle:
                'Классические архитектурные паттерны на живых схемах: как разложить систему на слои и компоненты, зачем это нужно и какую цену приходится платить.',
            categoryCount: (n) => `${n} ${pluralizeRu(n, 'паттерн', 'паттерна', 'паттернов')}`,
            comingSoon: 'Скоро',
            comingSoonHint: 'Раздел готовится - темы уже определены',
            categoryOpenHint: 'Открыть раздел',
            expand: 'Показать темы',
            ...
            sectionDeepDive: 'Как это работает',
            sectionDetailsWhenToUse: 'Нюансы выбора',
            sectionRealWorldDetails: 'Примеры и история',
            sectionDiagram: 'Схема',
            sectionImplementation: 'Реализация',
            sectionProsCons: 'Плюсы и минусы',
            sectionPros: 'Плюсы',
            sectionCons: 'Минусы',
            sectionQuiz: 'Проверь себя',
            sectionRelated: 'Похожие паттерны',
            tabsAriaLabel: 'Разделы паттерна',
            prev: 'Предыдущий',
            next: 'Следующий',
            walkthroughStep: (cur, tot) => `Шаг ${cur} / ${tot}`,
            walkthroughPrev: 'Назад',
            ...
            presentationAccordion: [
                {
                    title: 'Зачем разделять экран, данные и ввод',
                    blocks: [
                        { type: 'p', html: 'Когда экран, данные и обработка пользовательского ввода живут в одном классе, любое изменение дизайна рискует сломать бизнес-логику, а любой тест бизнес-правил тянет за собой весь UI. <strong>Презентационный слой</strong> - это семейство паттернов, решающих ровно эту проблему: они разводят три ответственности так, чтобы каждую можно было менять и тестировать отдельно.' },
                        { type: 'p', html: 'MVC, MVP и MVVM - три классических паттерна этого семейства, и все три ведут родословную к одной и той же идее <strong>Трюгве Реенскауга</strong> 1979 года: данные (Model) не должны знать об экране, а экран должен лишь отображать данные. Разница между тремя паттернами - не в этом общем правиле, а в двух конкретных вопросах.' },
                        { type: 'ul', items: [
                            '<strong>Кто общается с View напрямую:</strong> в MVC это Controller, обычно без всякой абстракции между ним и конкретным виджетом. В MVP - Presenter, но уже через абстрактный интерфейс View, который легко подменить тестовой заглушкой.',
                            '<strong>Как View узнаёт об изменениях:</strong> явный вызов метода отрисовки (Passive View), подписка через паттерн Observer, или автоматическое двустороннее связывание (data binding) в MVVM, где ViewModel вообще не знает о существовании View.',
                        ]},
                    ],
                },
                {
                    title: 'Как выбрать паттерн',
                    blocks: [
                        { type: 'p', html: 'Универсально лучшего варианта нет - выбор паттерна определяется тем, какие средства связывания данных и экрана предоставляет платформа.' },
                        { type: 'ul', items: [
                            '<strong>MVC</strong> - естественный выбор для серверных веб-фреймворков (Rails, Django, Spring MVC): Controller создаётся заново на каждый HTTP-запрос, долгоживущее состояние экрана там и не нужно.',
                            '<strong>MVP</strong> - когда Presenter обязан тестироваться юнит-тестами без реального UI: абстрактный интерфейс View позволяет подменить экран моком и проверить логику в изоляции.',
                            '<strong>MVVM</strong> - когда платформа сама умеет data binding (WPF/XAML, Angular, Vue, SwiftUI): ViewModel просто выставляет наблюдаемые свойства, а View подписывается на них декларативно, без ручного кода синхронизации.',
                        ]},
                        { type: 'p', html: 'Практическое правило: если платформа даёт готовый механизм связывания - берите MVVM. Если нужна тестируемость логики без готового биндинга - MVP. Если экран живёт ровно один запрос-ответ - классический MVC.' },
                    ],
                },
                {
                    title: 'О связывании данных',
                    blocks: [
                        { type: 'p', html: '<strong>Связывание данных (data binding)</strong> - механизм, которым View узнаёт об изменении Model или ViewModel. Он бывает разной степени автоматизации, и от выбора зависит, сколько кода придётся писать вручную.' },
                        { type: 'ul', items: [
                            '<strong>Одностороннее связывание (one-way binding):</strong> данные текут только от Model/ViewModel к View - типичная схема React или Vue при обычном рендере: состояние поменялось, компонент перерисовался, обратного пути нет.',
                            '<strong>Двустороннее связывание (two-way binding):</strong> View может напрямую менять Model/ViewModel в ответ на ввод пользователя - классика Angular (<code>ngModel</code>) и WPF, удобно для форм, но усложняет отслеживание источника изменения.',
                            '<strong>Явное уведомление (push через Observer):</strong> Model сама рассылает событие подписчикам при изменении - оригинальный механизм MVC в Smalltalk и <code>NSNotificationCenter</code> в Cocoa AppKit.',
                            '<strong>Ручной вызов (Passive View):</strong> ни подписки, ни автоматического биндинга нет вообще - Controller или Presenter явно вызывает метод View после каждого изменения, и это самый простой вариант для мокирования в тестах.',
                        ]},
                    ],
                },
                {
                    title: 'Как это классифицируется',
                    blocks: [
                        { type: 'p', html: 'Презентационные паттерны удобно сравнивать по нескольким независимым осям - это помогает быстро понять, чем один вариант отличается от другого, не читая полное описание каждого.' },
                        { type: 'ul', items: [
                            '<strong>По месту, где живёт решающая логика:</strong> Controller в MVC, Presenter в MVP, ViewModel в MVVM - в каждом паттерне это один и тот же по смыслу компонент под разным именем.',
                            '<strong>По механизму уведомления View:</strong> ручной вызов (Passive View), подписка Observer, автоматический data binding - от самого явного к самому неявному.',
                        ]},
                        { type: 'ul', items: [
                            '<strong>По тестируемости без реального UI:</strong> MVC сложнее всего - Controller обычно трогает View напрямую. MVP проще - интерфейс View подменяется моком. MVVM проще всего - ViewModel вообще не хранит ссылку на View.',
                            '<strong>По накладным расходам:</strong> MVC добавляет меньше всего кода на маленьком экране, MVVM требует инфраструктуры для observable-свойств, но окупается на экранах с большим количеством полей и сложной валидацией.',
                        ]},
                    ],
                },
            ],
    ...
    en: {
        ...
        architecturalPatterns: {
            ...
            catalogTitle: 'Architectural Patterns Academy',
            catalogSubtitle:
                'Classic architectural patterns on live diagrams: how to split a system into layers and components, why it matters, and what it costs.',
            categoryCount: (n) => `${n} pattern${n === 1 ? '' : 's'}`,
            comingSoon: 'Coming soon',
            comingSoonHint: 'This section is in progress - topics are already defined',
            categoryOpenHint: 'Open section',
            expand: 'Show topics',
            collapse: 'Collapse',
            sectionIntent: 'Intent',
            ...
            sectionDeepDive: 'How it works',
            sectionDetailsWhenToUse: 'Choice nuances',
            sectionRealWorldDetails: 'Examples and history',
            ...
            sectionCons: 'Cons',
            sectionQuiz: 'Test yourself',
            sectionRelated: 'Related patterns',
            tabsAriaLabel: 'Pattern sections',
            prev: 'Previous',
            next: 'Next',
            walkthroughStep: (cur, tot) => `Step ${cur} / ${tot}`,
            walkthroughPrev: 'Back',
            ...
            presentationAccordion: [
                {
                    title: 'Why split the screen, the data, and the input',
                    blocks: [
                        { type: 'p', html: 'When the screen, the data, and the user-input handling all live in one class, any design change risks breaking business logic, and any test of business rules drags the whole UI along with it. The <strong>presentation layer</strong> is a family of patterns that solves exactly this problem: they split the three responsibilities so each can change and be tested on its own.' },
                        { type: 'p', html: 'MVC, MVP, and MVVM - the three classic patterns in this family - all trace back to the same 1979 idea from <strong>Trygve Reenskaug</strong>: the data (Model) shouldn\'t know about the screen, and the screen should only display the data. The difference between the three isn\'t that shared rule - it\'s two specific questions.' },
                        { type: 'ul', items: [
                            '<strong>Who talks to the View directly:</strong> in MVC it\'s the Controller, usually with no abstraction between it and the concrete widget. In MVP it\'s the Presenter, but only through an abstract View interface that\'s easy to swap for a test double.',
                            '<strong>How the View learns about changes:</strong> an explicit render call (Passive View), an Observer-pattern subscription, or automatic two-way data binding in MVVM, where the ViewModel doesn\'t even know the View exists.',
                        ]},
                    ],
                },
                {
                    title: 'How to pick a pattern',
                    blocks: [
                        { type: 'p', html: 'There is no universally best option - the right pattern depends on what data-binding tools the platform actually gives you.' },
                        { type: 'ul', items: [
                            '<strong>MVC</strong> - the natural fit for server-side web frameworks (Rails, Django, Spring MVC): a fresh Controller is created per HTTP request, so there is no long-lived screen state to manage in the first place.',
                            '<strong>MVP</strong> - when the Presenter must be unit-tested without a real UI: the abstract View interface lets the screen be replaced with a mock and the logic verified in isolation.',
                            '<strong>MVVM</strong> - when the platform ships real data binding (WPF/XAML, Angular, Vue, SwiftUI): the ViewModel just exposes observable properties, and the View subscribes to them declaratively, with no hand-written sync code.',
                        ]},
                        { type: 'p', html: 'A practical rule: if the platform gives you a real binding mechanism, reach for MVVM. If you need testable logic without one, use MVP. If the screen only ever lives for one request-response cycle, plain MVC is enough.' },
                    ],
                },
                {
                    title: 'On data binding',
                    blocks: [
                        { type: 'p', html: '<strong>Data binding</strong> is the mechanism by which the View learns that the Model or ViewModel has changed. It comes in different degrees of automation, and the choice determines how much code you end up writing by hand.' },
                        { type: 'ul', items: [
                            '<strong>One-way binding:</strong> data flows only from the Model/ViewModel to the View - the typical React or Vue rendering flow: state changes, the component re-renders, and there is no path back.',
                            '<strong>Two-way binding:</strong> the View can change the Model/ViewModel directly in response to user input - classic Angular (<code>ngModel</code>) and WPF, convenient for forms but harder to trace the source of a change.',
                            '<strong>Explicit push via Observer:</strong> the Model itself broadcasts an event to subscribers when it changes - the original MVC mechanism in Smalltalk and <code>NSNotificationCenter</code> in Cocoa AppKit.',
                            '<strong>Manual call (Passive View):</strong> no subscription and no automatic binding at all - the Controller or Presenter explicitly calls a View method after every change, the simplest case to mock in tests.',
                        ]},
                    ],
                },
                {
                    ...
                    blocks: [
                        { type: 'p', html: 'Presentation-layer patterns are easiest to compare along a few independent axes - it helps to see quickly how one option differs from another without reading each pattern\'s full description.' },
                        { type: 'ul', items: [
                            '<strong>By where the decision logic lives:</strong> the Controller in MVC, the Presenter in MVP, the ViewModel in MVVM - the same conceptual component under a different name in each pattern.',
                            '<strong>By the View-notification mechanism:</strong> a manual call (Passive View), an Observer subscription, or automatic data binding - from the most explicit to the most implicit.',
                        ]},
                        { type: 'ul', items: [
                            '<strong>By testability without a real UI:</strong> MVC is hardest - the Controller usually touches the View directly. MVP is easier - the View interface gets replaced by a mock. MVVM is easiest - the ViewModel never holds a reference to the View at all.',
                            '<strong>By overhead:</strong> MVC adds the least code on a small screen, MVVM needs infrastructure for observable properties but pays off on screens with many fields and complex validation.',
                        ]},
                    ],
                },
            ],
            ...
            quizRetry: 'Try again',
            quizScore: (score, total) => `Your score: ${score} out of ${total}.`,
            quizPassTitle: 'Great job!',
            quizWarnTitle: 'Almost there',
            quizFailTitle: 'Not quite yet',
            quizPass: 'You handled this material confidently - keep it up!',
            quizWarn: 'Part of the material landed, but a few key ideas haven\'t stuck yet. Go through it again and retake the quiz.',
            quizFail: 'There are gaps worth closing. Go through the material again and retake the quiz - you\'ll get there.',
            quizHintShow: 'Show hint',
            quizHintHide: 'Hide hint',
            quizCorrect: 'Correct!',
            quizIncorrect: 'Incorrect.',
            quizReportIssue: 'Spotted a mistake in the quiz? Let us know',
        },
        error: {
